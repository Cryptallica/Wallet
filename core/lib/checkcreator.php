...
/****************************Check output METHODS****************************/
    ...
    public function createSendCheck($coin, $amount, $pub, $priv) {
        ...
        //Send coin
        $text = $this->createImageText(ucfirst($coin)."s"
            /* 20, 250, 50,  */
        );
        $text = imagerotate($text, 90, 0);
        ...
        $check = $this->imagePlace($check, $amount_im, 40, 25);
        $check = $this->imagePlace($check, $address_im, 7, 25);
        $check = $this->imagePlace($check, $label_im, 7, 50);
        
        //Back Address box
        $privatekeybox = $this->createRectangle(340, 100);
        
        //Back Address
        $privatetext_label = $this->createImageText("Private Key", 11, 320, 30, [200,200,200]);
        $privatetext_im = $this->createImageText($priv, 11, 320, 30, [255,255,255]);
        $qrprivwidth = (510 - (imagesx($qrimpriv)/2));
        
        
        //Back use private key
        $check = $this->imagePlace($check, $privatekeybox, 340, 550);
        $check = $this->imagePlace($check, $qrimpriv, $qrprivwidth, 490);
        $check = $this->imagePlace($check, $privatetext_label, 347, 600);
        $check = $this->imagePlace($check, $privatetext_im, 347, 620);
    ...
    private function createCheckBase($qrim, $coin) {
        ...
        $currencycenter = (170 - (imagesx($currency)/2));
        
        //Center qr image
        $qrimpubwidth = (170 - (imagesx($qrim)/2));
        
        //Create check front
        $check = $this->imagePlace($background, $black_box);
        $check = $this->imagePlace($check, $border, 339, 0);
        $check = $this->imagePlace($check, $qrim, $qrimpubwidth, 80);
        $check = $this->imagePlace($check, $currency, $currencycenter, 490);
