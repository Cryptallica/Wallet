...
/****************************SETUP METHODS****************************/
    ...
    function Cryptallica($forcelogin=false) {
        ...
        //get and loadCurrent logged in coin
        $this->getcoin();
        
        //force login for use in controllers
        if($forcelogin) {
            if(!$this->isLoggedIn()) {
                die("Must be logged in for these actions");
            }
        }
    }
    ...
    private function getconfig() {
        //Load config into associative array available_coins
        $this->available_coins = json_decode(
            file_get_contents(CRYPTALLICA_DIR."config.json"),
        true);
        
        if(!is_array($this->available_coins)) {die("Config not valid json");}
        
        //Load session data if not set
    ...
    private function connect($coinname='', $username='', $password='') {
        try {
            $port = $this->available_coins[$coinname]["port"];
...
/****************************Check Creationg METHODS****************************/   
    ...
    public function createSendCheck($password, $fromAccount, $amount=0, $json_data) {
        if(!$this->isLoggedIn()) {return false;}
    ...
    //create plain qr
    public function createRequestQr($toaccount, $amount, $label='', $message='') {
        if(empty($toaccount)) {
            $coin_address = $this->coin("rsrc")->getnewaddress();
        } else {
            $coin_address = $this->coin("rsrc")->getnewaddress($toaccount);
        } 
        
        $qrurl = $this->createUrlQR($coin_address, $amount, $label, $message);
        
        try {
            //Create check
            require_once(CRYPTALLICA_DIR."lib".DS."checkcreator.php");
            $check_creator = new CheckCreator();
            $qrim = $check_creator->createQR($qrurl);
    ...
    //Test requesting with dogecoin address for public and private
    public function createSendRequestTest() {
        //Create check
        try {
            require_once(CRYPTALLICA_DIR."lib".DS."checkcreator.php");
            $check_creator = new CheckCreator();
            $check = $check_creator->createSendCheck("litecoin", 10, "DLXvYNQGHF8sfGHCHtKrazrjBWa6HGeigF", "DLXvYNQGHF8sfGHCHtKrazrjBWa6HGeigF");
            header('Content-Type: image/png');
            imagepng($check);
            imagedestroy($check);
        } 
